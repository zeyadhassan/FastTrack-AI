...
class GraphManager {
     ...
     */
    findUntestedCodeBlocks() {
        const codeBlocks = this.graph.nodes.filter(node => node.type === models_1.NodeType.CodeBlock);
        return codeBlocks.filter(codeBlock => {
            // Find edges where this code block is the source and the edge type is TestedBy
            const testedByEdges = this.graph.edges.filter(edge => edge.source === codeBlock.id && edge.type === models_1.EdgeType.TestedBy);
     ...
     */
    findStaleAIGeneratedBlocks(days) {
        const now = new Date();
        const codeBlocks = this.graph.nodes.filter(node => node.type === models_1.NodeType.CodeBlock);
        return codeBlocks.filter(codeBlock => {
            // Check if the code block is AI-generated
            const aiGeneratedEdges = this.graph.edges.filter(edge => edge.target === codeBlock.id && edge.type === models_1.EdgeType.Generated);
     ...
     */
    findFilesWithMostAICode(limit = 10) {
        // Map to count AI-generated code blocks per file
        const fileCounts = new Map();
        // Find all AI-generated code blocks
        const aiGeneratedEdges = this.graph.edges.filter(edge => edge.type === models_1.EdgeType.Generated);
        ...
        for (const edge of aiGeneratedEdges) {
            const codeBlock = this.graph.nodes.find(node => node.id === edge.target && node.type === models_1.NodeType.CodeBlock);
            if (codeBlock) {
     ...
     */
    createDeveloperNode(id, name, email) {
        const node = {
            id,
     ...
     */
    createAISuggestionNode(id, aiTool) {
        const timestamp = new Date().toISOString();
        const node = {
            id,
            type: models_1.NodeType.AI_Suggestion,
            aiTool,
            timestamp,
            createdAt: timestamp,
            updatedAt: timestamp
     ...
     */
    createCodeBlockNode(id, fileId, startLine, endLine, content) {
        const timestamp = new Date().toISOString();
        const node = {
            id,
     ...
     */
    createFileNode(id, path, language) {
        const node = {
            id,
     ...
     */
    createTestNode(id, path, testedCodeBlockIds = []) {
        const node = {
            id,
            type: models_1.NodeType.Test,
            path,
            testedCodeBlockIds,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
     ...
     */
    createCommitNode(id, hash, message, authorId) {
        const timestamp = new Date().toISOString();
        const node = {
            id,
            type: models_1.NodeType.Commit,
            hash,
            message,
            timestamp,
            authorId,
     ...
     */
    createEdge(id, type, sourceId, targetId) {
        const edge = {
            id,
            type,
            source: sourceId,
